...
class CRF(LightningModule):
    ...
    def __init__(self, label2id: Dict[str, int]) -> None:
        ...
        self.transitions.detach()[label2id[START_TAG], :] = -10000

        self.transitions.detach()[:, label2id[STOP_TAG]] = -10000
...
class ViterbiLoss(LightningModule):
    ...
    def __init__(self, label2id: Dict[str, int]) -> None:
        ...
        self.tagset_size = len(label2id)
        self.start_tag = label2id[START_TAG]
        self.stop_tag = label2id[STOP_TAG]

    def forward(self, features: Tensor, lengths, transitions, targets: torch.Tensor) -> torch.Tensor:
        ...
        scores_upto_t = torch.zeros(batch_size, self.tagset_size, device=self.device, dtype=features.dtype)
    ...
    def _format_targets(self, targets: torch.Tensor, lengths: torch.IntTensor):
        ...
        for t in targets_per_sentence:
            t += [self.label2id[STOP_TAG]] * (int(lengths.max().item()) - len(t))

        matrix_indices = [
            [self.label2id[START_TAG] + (s[0] * self.tagset_size)]
...
class ViterbiDecoder(LightningModule):
    ...
    def decode(
        ...
        scores_upto_t = torch.zeros(batch_size, seq_len + 1, self.tagset_size, dtype=features.dtype).to(self.device)
