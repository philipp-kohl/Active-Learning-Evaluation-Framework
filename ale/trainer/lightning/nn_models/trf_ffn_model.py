...
class TransformerFfnLightning(LightningModule):
    ...
    def forward(self, input_ids, attention_mask, labels=None, **kwargs):
        ...
        cls_per_token = self.linear(embeddings.last_hidden_state)

        loss = 0.0
        if labels is not None:
            loss = self.loss_function(cls_per_token.view(-1, self.num_labels), labels.view(-1))

        return cls_per_token, loss

    def training_step(self, batch, batch_idx):
        cls_per_token, loss = self(**batch)
    ...
    def evaluate(self, batch, logits, metrics: Dict[str, Metric], f1_per_label: Metric):
        mask = batch["attention_mask"]
