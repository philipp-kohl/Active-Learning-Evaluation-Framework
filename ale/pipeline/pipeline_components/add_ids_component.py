...
class AddIdsTrainComponent(PipelineComponent):
...
class AddIdsDevComponent(PipelineComponent):
    def prepare_run(self):
        data_cfg = self.pipeline_storage.cfg.data
        path = create_path(data_cfg.data_dir, data_cfg.dev_file, data_cfg.file_format)
